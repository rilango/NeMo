...
old_qn="/QuartzNet15x5Base-Ensubset_old/all_manifest.json"
python /home/ebakhturina/NeMo/examples/asr/transcribe_speech.py \
  $ARG_MODEL=$MODEL_NAME_OR_PATH \
  dataset_manifest=${BASE_DIR}/${old_qn} \
  output_filename=${BASE_DIR}/${old_qn}_conformer.json \
  batch_size=4 || exit

# calculate metrics
python /home/ebakhturina/misc_scripts/ctc_segmentation/benchmark/calc_metrics.py \
--input=${BASE_DIR}/${old_qn}_conformer.json \
--output=${BASE_DIR}/${old_qn}_conformer_metrics.json \
--asr_pred=pred_text

# filter
python /home/ebakhturina/misc_scripts/ctc_segmentation/benchmark/agg_metrics.py \
--manifest=${BASE_DIR}/${old_qn}_conformer_metrics.json \
--audio_dir=/home/ebakhturina/data/segmentation/benchmark/DEL/audio/